...
`eternal-role` pins one role to one member (`eternalRoles`, currently one entry in the melting pot). It rides the per-guild `roles-sweep` and the role/member gateway handlers, all funnelling into `enforceEternalRoles`, which re-derives everything from live state so double-firing settles in one pass. Removed from the member ⇒ given back; deleted ⇒ recreated from the stored copy (name, colour, hoist, mentionable, permissions, emoji, **icon bytes**) and the copy retargeted; edited ⇒ the edited role is left alone, a fresh copy is created, assigned, and placed directly above it. **The copy is captured once and never re-captured from Discord**, or disable-edit-enable would launder an admin's edit into the eternal version; only merlin's own recreate moves `role_id`/`icon_hash`. It **yields to jail**, or it would loop against `HandleMemberUpdate`'s re-strip. Position is never compared (it moves whenever any role is reordered). Changing the eternal version means the operator deleting the `script_eternal_roles` row; there is no command for it yet.
