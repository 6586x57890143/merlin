...
- **Details stay `key=value` in the row and render as labelled lines in the embed** (`audit.readable`): `user=<@1> reason="x"` becomes one `**Member** <@1>` line per pair, with prose kept in order and anything that is not the pair format passed through untouched. Lists of IDs go in as `[%s]` of `core.MentionRoles`/`MentionUsers`, never `%v` of a slice, which is how raw snowflakes reached the channel. A message is a `core.MessageLink`, and where merlin has just deleted it the link is struck through and says so (`aimod.messageFate`), since Discord renders a dead jump link as "Unknown message".
