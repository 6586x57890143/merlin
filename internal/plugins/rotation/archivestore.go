...
// archives, which the sweep never selects.
...
func (s *pgArchiveStore) Insert(ctx context.Context, rec ArchiveRecord) error {
	_, err := s.pool.Exec(ctx, `
		INSERT INTO rotation_archives (channel_id, guild_id, source_channel_id, archived_at, delete_after)
		VALUES ($1, $2, $3, $4, $5)
		ON CONFLICT (channel_id) DO UPDATE
			SET archived_at = $4, delete_after = $5
	`, rec.ChannelID, rec.GuildID, rec.SourceChannelID, rec.ArchivedAt, rec.DeleteAfter)
	if err != nil {
...
func (s *pgArchiveStore) DueForDeletion(ctx context.Context, guildID string, now time.Time) ([]ArchiveRecord, error) {
	rows, err := s.pool.Query(ctx, `
		SELECT channel_id, guild_id, source_channel_id, archived_at, delete_after
	...
	for rows.Next() {
		...
		if err := rows.Scan(&rec.ChannelID, &rec.GuildID, &rec.SourceChannelID, &rec.ArchivedAt, &rec.DeleteAfter); err != nil {
